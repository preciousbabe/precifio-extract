...
async function extractZipDocuments(zipBuffer) {
  ...
  const entries = zip
    ...
    .filter(
      (entry) =>
        !entry.isDirectory &&
        isSupportedFile(entry.entryName)
    );
...
export async function processBatch({
  ...
  const documents =
    await extractZipDocuments(zipBuffer);

  const totalDocuments =
    documents.length;

  const totalPages =
    await calculateBatchPages(
      documents
    );
...
export async function calculateZipCredits(
  zipBuffer
) {
  const documents =
    await extractZipDocuments(
      zipBuffer
    );
