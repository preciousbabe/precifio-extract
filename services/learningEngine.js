import { supabaseAdmin } from '../config/supabase.server.js'
...
export async function learnFromCorrection(
  ...
  // Reward user
  const reward = await rewardCorrection(
    userId,
    extractionId,
    fieldName
  )
  ...
  if (fieldName === 'vendor_name' || fieldName === 'category') {
    ...
    if (vendorName) {
      await updateVendorPattern(userId, vendorName, {
        category:
          fieldName === 'category' ? correctedValue : undefined,
        source: 'correction'
      })
    }
  }
...
export async function updateVendorPattern(
  userId,
  vendorName,
  updates
) {
  ...
  if (existing) {
    ...
    const newAvg = existing.average_invoice_amount
      ? ((existing.average_invoice_amount *
    ...
    await supabaseAdmin
      ...
      .update({
        ...
        average_invoice_amount:
          newAvg || existing.average_invoice_amount,
        last_seen_at: new Date().toISOString()
      })
      .eq('id', existing.id)
  } else {
    await supabaseAdmin.from('vendor_patterns').insert({
      user_id: userId,
...
export async function applyVendorPatterns(userId, extraction) {
  ...
  const normalized = extraction.vendor_name
    .toLowerCase()
    .trim()
    .replace(/[^a-z0-9]/g, '')
  ...
  return {
    ...
    vendor_name:
      extraction.vendor_name || pattern.vendor_name,
...
export async function detectAnomalies(
  ...
  if (extraction.vendor_name && extraction.total_amount) {
    ...
    if (
      pattern?.average_invoice_amount &&
      pattern.invoice_count > 2
    ) {
      const threshold =
        pattern.average_invoice_amount * 2.5
      ...
      if (extraction.total_amount > threshold) {
        alerts.push({
          ...
          severity: 'warning',
          description: `${extraction.vendor_name}: ${Math.round(
            extraction.total_amount /
              pattern.average_invoice_amount
          )}x average ($${pattern.average_invoice_amount})`
        })
      }
    }
  }
