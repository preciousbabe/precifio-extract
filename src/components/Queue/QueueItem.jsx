...
export default function QueueItem({ item, queue }) {
  ...
  const isRecoverable = isFailed && !!item.extractionId && item.needsAction !== "retry-fresh";
  const needsFreshStart = isFailed && item.needsAction === "retry-fresh";
  const canRemove = !queue.processing || isCompleted || isFailed;

  const isGuestLimitError = item.error && (
    item.error.includes('GUEST_LIMIT_REACHED') || item.error.includes('Free extraction used')
  );

  const handleView = () => {
    if (isGuest && isGuestLimitError) { requireAuth('signup'); return; }
    ...
    <div className={`queue-item ${item.status}`}>
      <div className="queue-item-header">
        <div className="queue-item-left">
          <input type="checkbox" className="queue-item-checkbox" checked={item.selected || false}
            onChange={() => queue.toggleSelection(item.id)} />
          <div className="queue-item-info">
            <strong>{item.name}</strong>
            <div className="queue-meta">
              {(item.size / 1024).toFixed(1)} KB
              {isGuest && isCompleted && <span className="guest-badge">Free Preview</span>}
            </div>
          </div>
        </div>
        ...
        <div className="queue-actions">
          {(isCompleted || isFailed) && (
            <button onClick={handleView}
              ...
              {isGuestLimitError ? '🔒 Sign Up to View' : isCompleted ? 'View Results' : 'View Error'}
            </button>
          )}

          {isFailed && !isGuestLimitError && (
            <button onClick={() => queue.retryItem(item.id)} className="queue-btn warning">
              {isRecoverable ? "↻ Recover Result" : needsFreshStart ? "↻ Retry Fresh" : "↻ Retry"}
            </button>
          )}
